...
interface HourlyTemperatureProps { data: ForecastData; }
interface ChartData { time: string; temp: number; feels_like: number; }

const CustomTooltip: React.FC<any> = ({ active, payload }) => {
  if (!active || !payload || !payload.length) return null;
  const temp = payload.find((p: any) => p.dataKey === 'temp')?.value;
  const feels = payload.find((p: any) => p.dataKey === 'feels_like')?.value;
...
const HourlyTemperature: React.FC<HourlyTemperatureProps> = ({ data }) => {
  const chartData: ChartData[] = useMemo(() => {
    ...
    return list.slice(0, 8).map((item: any) => ({
      time: format(new Date(item.dt * 1000), 'h a'), // "6 AM"
      temp: Math.round(item.main.temp),
      feels_like: Math.round(item.main.feels_like),
    }));
  }, [data]);
